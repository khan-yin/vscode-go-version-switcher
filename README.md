...
> **2026-05-03** — Fixed `gvm list` output parsing: the original regex only matched lines starting with `goX.Y`, which missed some GVM output formats. Now correctly extracts version strings like `goX.Y.Z` from any position in the line. The icon generated by [ChatGPT Images 2.0](https://chatgpt.com).
